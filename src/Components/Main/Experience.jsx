...
function Experience() {
    ...
    const workplaces = [
      ...
      },
      {
        ...
        description: ["Assist new and current students coming to the Campus", "Make sure everyone coming to the campus follow the safety policy.","Work in heavy-teamwork environment", "Got asked to return to the job twice"],
      ...
      },
      {
        ...
        description: ["Work and communicate with team to deliver exceptional customer service", "Assist and guide customers to the correct articles","Communicate to understand customer’s need", "Received positive feedbacks from employers"],
  ...
  return (
    <section id="experience" className={`container-fluid ${styles.section}`}>
        ...
        <div id = {styles.experience} className="col-xl-7 col-9 mx-auto d-lg-flex d-block border border-success border-5 p-3 rounded-4 bg-dark text-light">
          <div id={styles.list} className='d-flex flex-lg-column justify-content-center col-lg-3 col mx-lg-0 mb-3 mb-lg-0 pb-sm-0 pb-3'>
            {workplaces.map(workplace => 
              <button id={workplace.place} key={workplace.place} type="button" className="btn btn-primary d-inline rounded-0 my-lg-1 mx-lg-0 mx-1" onClick={(e) => changeActiveDesc(e.target.id)}>{workplace.place}</button>
            )}
          </div>
          <div id = {styles['active-desc']} className='border-warning'>
              <h3>{currentActiveDesc.position}</h3>
              <p>From {convertDateToString(currentActiveDesc.timeStart)} to {convertDateToString(currentActiveDesc.timeEnd)}</p>
              <ul>
                {currentActiveDesc.description.map((desc,idx) => <li key={idx}>{desc}</li>)}
              </ul>
